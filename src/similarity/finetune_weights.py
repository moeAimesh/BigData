...
try:
    ...
    raise SystemExit("Fehlt: embedding_vec.extract_embeddings (wie in deiner Pipeline).")
...
def _load_uint8_bgr(path: Path) -> np.ndarray:
    ...
    if img is None: raise ValueError(f"Kann Bild nicht lesen: {path}")
    return img

def _preprocess_for_model(img_bgr: np.ndarray) -> np.ndarray:
    return cv2.resize(img_bgr, TARGET_SIZE, interpolation=cv2.INTER_LINEAR).astype(np.float32)/255.0
...
def _load_rgb01_and_hsv_u8(p: Path, size=(224,224)):
    ...
    if bgr is None: raise ValueError(f"Bild nicht lesbar: {p}")
...
def _hsv_hist_intersection(hsv_a: np.ndarray, hsv_b: np.ndarray,
                           h_bins=32, s_bins=32, v_bins=32) -> float:
    ...
    def _l1n(x): s=x.sum(); return x/(s+1e-9)
...
def list_tables(conn)->List[str]:
    cur=conn.cursor()
    cur.execute("SELECT name FROM sqlite_master WHERE type='table' AND name LIKE ? ORDER BY name ASC",(f"{TABLE_PREFIX}%",))
...
def fetch_rows(conn, t, batch=20000):
    cur=conn.cursor()
    cur.execute(f'SELECT {COL_ID},{COL_IMAGE_PATH},{COL_EMB32_PATH},{COL_EMB64_PATH},{COL_EMB512_PATH} FROM "{t}"')
    while True:
...
def rerank64(q64: np.ndarray, cand_meta, topY:int):
    ...
    for i, (_,_,_,_,p64,_) in enumerate(cand_meta):
        try:
            ...
            mats.append(v); idxs.append(i)
        except Exception:
            continue
    if not mats: return []
    M=np.vstack([l2n(v) for v in mats]); q=l2n(q64)
    sims=M @ q
...
def rerank512(q512: np.ndarray, cand_meta_pairs, topM:int):
    ...
    for cand_idx, meta in cand_meta_pairs:
        ...
        try:
            ...
            v=v[0] if (v.ndim==2 and v.shape[0]==1) else v
            mats.append(v.astype(np.float32).ravel()); idxs.append(cand_idx)
        except Exception:
            continue
    if not mats: return []
    M=np.vstack([l2n(v) for v in mats]); q=l2n(q512)
    sims=M @ q
    order=np.argsort(-sims)[:topM]
    return [(idxs[int(i)], float(sims[int(i)])) for i in order]

def get_fine_list_for_query(qpath: Path, pca: PCATransformer, idx32: HNSW32,
                            X:int, Y:int, M:int, k:int) -> List[Dict[str,Any]]:
    q512=_query512(qpath); q64=pca.transform64(q512); q32=q64[:32]
...
def fusion_score(cos512: float, lpips_val: Optional[float], ssim_val: Optional[float], color_val: Optional[float],
                 w_cos: float, w_lpips: float, w_ssim: float, w_color: float) -> float:
...
def mine_positives_for_query(qpath: Path, pca: PCATransformer, idx32: HNSW32,
                             X:int, Y:int, M:int, k:int,
                             use_rnn: bool=False, rnn_top:int=20) -> List[str]:
    fine_list = get_fine_list_for_query(qpath, pca, idx32, X,Y,M,k)
    if not fine_list: return []
    ...
    if use_rnn and positives:
        ...
        for p in positives:
            try:
                rev = get_fine_list_for_query(Path(p), pca, idx32, X=5, Y=3, M=2, k=rnn_top)
...
# ===== Grid =====
def grid(weights_grid: Dict[str, List[float]]) -> List[Tuple[float,float,float,float]]:
...
def sample_queries_from_db(db_path: Path, n: int, ext_filter=True) -> List[Path]:
    ...
    with sqlite3.connect(str(db_path)) as conn:
        ...
        for t in tabs:
            ...
            cur = conn.cursor()
            cur.execute(f'SELECT {COL_IMAGE_PATH} FROM "{t}" ORDER BY RANDOM() LIMIT ?', (want,))
        ...
        if len(paths) < n:
            ...
            t0 = tabs[0]
            cur = conn.cursor()
            cur.execute(f'SELECT {COL_IMAGE_PATH} FROM "{t0}" ORDER BY RANDOM() LIMIT ?', (extra,))
            for (p,) in cur.fetchall():
                paths.append(Path(p))
...
# ===== Haupt-Eval =====
def run_eval(db_path: Path, ipca_path: Path, idx_dir: Optional[Path],
             queries_jsonl: Optional[Path], queries_dir: Optional[Path], queries_from_db: Optional[int],
             out_json: Path, out_mined_jsonl: Optional[Path],
             k:int, maxq:int, X:int, Y:int, M:int,
             weights_grid: Dict[str,List[float]],
             disable_lpips: bool, disable_ssim: bool,
             use_rnn: bool) -> Dict[str,Any]:
    ...
    if queries_jsonl:
        ...
        mined=[]
        for q in q_files:
            pos = mine_positives_for_query(q, pca, idx32, X,Y,M,k, use_rnn=use_rnn, rnn_top=20)
            if pos:
        ...
        if not mined:
            raise SystemExit("Auto-Mining (DB) fand keine Positiven – Schwellen senken oder X/Y/M erhöhen.")
        eval_rows = mined
        if out_mined_jsonl:
        ...
        allowed = {".jpg",".jpeg",".png",".bmp",".webp",".tif",".tiff"}
        q_files = [p for p in Path(queries_dir).rglob("*") if p.is_file() and p.suffix.lower() in allowed]
        random.shuffle(q_files)
        q_files = q_files[:maxq]
        if not q_files:
            raise SystemExit("Keine Query-Bilder in --queries_dir gefunden (auch rekursiv nicht).")
        ...
        if not mined:
            raise SystemExit("Auto-Mining fand keine Positiven – Schwellen senken oder X/Y/M erhöhen.")
        eval_rows = mined
        if out_mined_jsonl:
            with open(out_mined_jsonl, "w", encoding="utf-8") as f:
                for r in mined:
    ...
    for (w_cos, w_lp, w_ss, w_col) in combos:
        ...
        for (qpath,pos,fine_list) in per_query_data:
            if not fine_list:
                pks.append(0.0); continue
            try:
                q_rgb01, q_hsv = get_img_feats(qpath)
            except Exception:
                sorted_paths = [it["image_path"] for it in sorted(fine_list, key=lambda d: -d["score"])]
                pks.append(precision_at_k(sorted_paths, pos, k)); continue
            ...
            for it in fine_list:
                ...
                try:
                    ...
                    if _HAS_LPIPS: lp_val = _lpips_dist(q_rgb01, c_rgb01)
                    if _HAS_SSIM:  ss_val = _ssim_sim (q_rgb01, c_rgb01)
                    col_val = _hsv_hist_intersection(q_hsv, c_hsv)
                except Exception:
                    pass
                final = fusion_score(cos512, lp_val, ss_val, col_val, w_cos, w_lp, w_ss, w_col)
        ...
        results.append({"weights":{"cos":w_cos,"lpips":w_lp,"ssim":w_ss,"color":w_col},"P@k":mean_pk})

    results.sort(key=lambda r: -r["P@k"])
    out = {
        "k": k,
        "mode": ("supervised" if queries_jsonl else ("db" if queries_from_db else "weak_labels")),
...
def main():
    ...
    # EINE von drei:
    ap.add_argument("--queries", type=str, default=None, help="JSONL mit {query, positives}")
    ap.add_argument("--queries_dir", type=str, default=None, help="Ordner mit Query-Bildern (rekursiv)")
    ap.add_argument("--queries_from_db", type=int, default=None, help="N zufällige Bilder direkt aus der DB als Queries")

    # Index:
    ap.add_argument("--index_dir", type=str, default=None, help="Ordner mit hnsw_32d.bin + labels_meta.pkl")
    ...
    if modes != 1:
        raise SystemExit("Gib GENAU EINES an: --queries ODER --queries_dir ODER --queries_from_db N")
    ...
    out = run_eval(
        ...
        out_mined_jsonl=Path(args.out_mined) if args.out_mined else (Path(args.out).with_suffix(".mined.jsonl") if args.queries_dir or args.queries_from_db else None),
        k=args.k, maxq=args.maxq, X=args.x, Y=args.y, M=args.m,
        weights_grid=wgrid,
