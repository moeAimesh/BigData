...
- Query-Embedding exakt wie im Ingest: (H,W,3) uint8 -> resize -> float32 [0..1] -> an extract_embeddings([...]).
  (extract_embeddings wendet intern erneut /255 & ImageNet-Standardisierung an  das entspricht DB-Pipeline.)
- 32D = erste 32 Komponenten von 64D (q64_raw[:32])  so wie pca_32 entstanden ist.
...
IPCA_PATH = (
    MODELS_DIR / "ipca.joblib"
)  # IncrementalPCA – exakt dasselbe Modell wie im DB-Build für 64D!
...
UMAP_TRANSFORM_USE_L2 = {
    32: False,
    64: False,
    512: False,
}  # metric='euclidean' -> False, metric='cosine' -> True
...
COL_ID = "id"
COL_IMAGE_PATH = "path"
COL_EMB32_PATH = "pca_32"  # 32D .npy (erste 32 Komponenten von 64D PCA)
...
def _fast_load_bytes_bgr(p: Path) -> np.ndarray:
    """Robustes Lesen über Bytes + imdecode (BGR). Keine Farbkonvertierung  wir replizieren Ingest-Annahme."""
...
def preprocess_for_model(img_uint8: np.ndarray, target_size=TARGET_SIZE) -> np.ndarray:
    ...
    if PIL_AVAILABLE:
        im = Image.fromarray(
            img_uint8
        )  # interpretiert Array wie es kommt; in Pipeline war das konsistent
        im = im.resize(target_size, Image.BILINEAR)
        arr = np.asarray(im, dtype=np.float32) / 255.0
    else:
        arr = (
            cv2.resize(img_uint8, target_size, interpolation=cv2.INTER_LINEAR).astype(
                np.float32
            )
            / 255.0
        )
...
def to_embed_dtype(arr: np.ndarray) -> np.ndarray:
    # In Ingest war EMBED_INPUT_DTYPE="float32" – einfach durchreichen.
...
class HNSW32:
    ...
    def add_batch(
        self, vecs32: np.ndarray, metas: List[Tuple[str, int, str, str, str, str]]
    ):
...
def table_has_columns(
    conn: sqlite3.Connection, table: str, cols: Tuple[str, str]
) -> bool:
    cur = conn.cursor()
...
def fetch_umap_coords(
    conn: sqlite3.Connection, table: str, ids: List[int], cols: Tuple[str, str]
...
def build_index_if_needed(db_path: Path) -> HNSW32:
    ...
    if INDEX_BIN.exists() and INDEX_META.exists():
        try:
            ...
            if db_total == index_count == meta_len:
                ...
                print(
                    f"[WARN] Index stale? db_total={db_total:,}, index={index_count:,}, meta={meta_len:,} → rebuild"
                )
        except Exception:
            need_rebuild = True
    ...
    with sqlite3.connect(str(db_path)) as conn:
        ...
        for t in tables:
            vecs, metas = [], []
            for rid, img_path, emb32_path, emb64_path, emb512_path in fetch_rows_iter(
                conn, t
            ):
                try:
                    ...
                    if v32.shape[0] < 32:
                        continue
                    # 32D sind die ersten 32 aus dem 64D – in Dateien schon 32D abgelegt
                    vecs.append(v32[:32].astype(np.float32))
                    metas.append(
                        (t, rid, img_path, emb32_path, emb64_path, emb512_path)
                    )
                except Exception:
                    continue
                if len(vecs) >= 20000:
...
def rerank_fine_512(
    q512: np.ndarray,
    cand_meta: List[Tuple[int, Tuple[str, int, str, str, str, str]]],
    topM: int,
...
def load_umap_models_separate() -> Dict[int, Any]:
    models: Dict[int, Any] = {}
    try:
        if UMAP32_PATH and UMAP32_PATH.exists():
            models[32] = load(UMAP32_PATH)
    except Exception:
        pass
    try:
        if UMAP64_PATH and UMAP64_PATH.exists():
            models[64] = load(UMAP64_PATH)
    except Exception:
        pass
    try:
        if UMAP512_PATH and UMAP512_PATH.exists():
...
def search_image(image_path: Path, db_path: Path) -> Dict[str, Any]:
    ...
    img_arr = _fast_load_bytes_bgr(
        image_path
    )  # (H,W,3) BGR uint8 – wie in Pipeline angenommen
    ...
    embs = extract_embeddings(
        [embed_in]
    )  # -> (1,512); intern erneut /255 & ImageNet-Norm
    ...
    if DEBUG_SELF_CHECK:
        with sqlite3.connect(str(db_path)) as conn:
            cur = conn.cursor()
            cur.execute(
                f'SELECT {COL_EMB512_PATH} FROM "{TABLE_PREFIX}1" WHERE {COL_IMAGE_PATH}=? LIMIT 1',
                (str(image_path),),
            )
            row = cur.fetchone()
    ...
    q32_raw = q64_raw[:32]  # (32,)  ← entspricht pca_32 Definition
    ...
    cand_meta = [
        index32.labels_meta[int(lbl)] for lbl in labels
    ]  # (table,id,img,emb32,emb64,emb512)
