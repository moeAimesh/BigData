...
- Query-Histogramm: nutzt calc_histogram()-Funktion (BGR, 3 x bins)
- DB-Histogramme: TEXT (csv) mit 96 Werten (3 x 32 Bins), L1-normalisiert
...
try:
    ...
    def calc_histogram(img, bins=32):
        ...
        for ch in (0, 1, 2):  # B,G,R
            h = (
                cv2.calcHist([img], [ch], None, [bins], [0, 256])
                .ravel()
                .astype(np.float32)
            )
            hists.append(h)
...
FUSION_WEIGHTS_MANUAL = (
    {  # final = w_cos*cos512 - w_lpips*LPIPS + w_ssim*SSIM + w_color*COLOR
        "cos": 0.65,
        "lpips": 0.25,
        "ssim": 0.10,
        "color": 0.10,
...
def fetch_color_hist_bulk_text(
    conn: sqlite3.Connection, table: str, ids: List[int], col: str = HIST_COL
) -> Dict[int, np.ndarray]:
    if not ids:
        return {}
...
def _ssim_sim(a_rgb01, b_rgb01) -> Optional[float]:
    ...
    try:
        return float(
            np.clip(ssim(a_rgb01, b_rgb01, data_range=1.0, channel_axis=2), 0, 1)
        )
    except Exception:
        return None
    ...
    if FUSION_ENABLED:
        ...
        with sqlite3.connect(str(db_path)) as conn:
            for table, ids in table_to_ids.items():
                id2hist_per_table[table] = fetch_color_hist_bulk_text(
                    conn, table, ids, HIST_COL
                )
        ...
        cands_lpips = sorted(
            cands_ssim, key=lambda d: float(d.get("sim_ssim", 0.0) or 0.0), reverse=True
        )[:T_lpips]
        ...
        for it in fine_top_m:
            ...
            it["score_fused"] = _fusion_score(
                cos512, lp_val, ss_val, col_val, FUSION_WEIGHTS
            )
            fused.append(it)
