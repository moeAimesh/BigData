...
def main():
    ...
    p.add_argument(
        "--mode",
        default="mix",
        choices=["mix", "embedding", "color", "hash"],
        help="Suchmodus",
    )
    p.add_argument("--top-k", type=int, default=5, help="Anzahl finaler Treffer")
    p.add_argument(
        "--hist-col", default="color_hist", help="Histogramm-Spaltenname in der DB"
    )
    p.add_argument(
        "--color-bins", type=int, default=32, help="Bins pro Kanal (Default 32)"
    )
    args = p.parse_args()
