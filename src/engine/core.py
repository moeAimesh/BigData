...
def _wrap_final(
    items: List[Dict[str, Any]], query_path: Path, mode: str, k: int
) -> Dict[str, Any]:
    return {
...
def warmup(cfg: Config) -> None:
    _ = (
        mix_search,
        emb_search,
        compute_query_hashes,
        search_by_hash_voting_multitables,
        calc_histogram,
    )
...
def _load_cached_table(
    cache_dir: Path, table: str
) -> Tuple[np.memmap, np.ndarray, np.ndarray] | Tuple[None, None, None]:
...
def _build_cache_for_table(
    db_path: Path, table: str, hist_col: str, bins: int, cache_dir: Path
):
    ...
    if (
        any(p.exists() for p in (P["mm"], P["ids"], P["paths"], P["meta"]))
        and not P["ok"].exists()
    ):
        import time
    ...
    with sqlite3.connect(str(db_path)) as conn:
        cur = conn.cursor()
        cur.execute(
            f'SELECT COUNT(*) FROM "{table}" WHERE "{hist_col}" IS NOT NULL AND TRIM("{hist_col}")<>""'
        )
        ...
        cur.execute(
            f'SELECT id, "{hist_col}", path FROM "{table}" WHERE "{hist_col}" IS NOT NULL AND TRIM("{hist_col}")<>""'
        )
        i = 0
        BATCH = 20000
        ...
        del mm
        if i < N:
            data = np.memmap(P["mm"], mode="r", dtype=np.float32, shape=(N, D))[
                :i
            ].copy()
            data.tofile(P["mm"])
            meta = {"N": i, "D": D}
...
def _search_color_cached_core(
    ...
    for t in _color_tables(dbp, table_prefix):
        ...
        if M is None:
            M, ids, paths = _build_cache_for_table(
                dbp, t, hist_col_text, bins, cache_dir
            )
        ...
        for i_local in range(Kp):
            item = (
                float(ff[i_local]),
                t,
                int(idx_local[i_local]),
                int(ids_loc[i_local]),
                str(paths_loc[i_local]),
                (
                    float(fh[i_local]),
                    float(fc2[i_local]),
                    float(fi[i_local]),
