...
def voxel_downsample_rgb(pos, bins_per_axis=64):
    ...
    keys = (idx[:,0] + bins_per_axis*idx[:,1] + (bins_per_axis**2)*idx[:,2]).astype(np.int64)
    ...
    for i, (s, e) in enumerate(zip(start_idx, end_idx)):
        ...
        count = e - s
        sizes[i] = 2.0 + 3.0*np.log1p(count)  # größere Voxel dichter belegt -> größerer Marker
