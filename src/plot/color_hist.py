...
def avg_rgb_from_path(p: str):
    try:
        ...
        if THUMB:
            ...
            if max(h0, w0) > THUMB:
                s = THUMB / max(h0, w0)
                img = cv.resize(img, (int(w0*s), int(h0*s)), interpolation=cv.INTER_AREA)
...
canvas = scene.SceneCanvas(keys='interactive', bgcolor='white', size=(1100, 800), show=True)
...
markers.set_data(
    ...
    size=POINT_SIZE, edge_width=0.0
)
